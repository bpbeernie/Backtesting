    ...
    def proccessDate(self, dateToProcess):
        ...
        for newBar in data:                        
            if not openBar:
                ...
                entryLimitforLong = adjustedLow
                profitTargetForLong = adjustedHigh
                stopLossForLong = adjustedLow - adjustedDiff
                
                entryLimitforShort = adjustedHigh
                profitTargetForShort = adjustedLow
                stopLossForShort = adjustedHigh + adjustedDiff

            else:
                # long
                if not longEntryRunning and not shortEntryRunning:
                    if newBar.low < entryLimitforLong:
                        longEntryRunning = True
                    elif newBar.high > entryLimitforShort:
                    ...
                    if longEntryRunning:
                        if newBar.high > profitTargetForLong:
                            ...
                            longEntryRunning = False
                            shortEntryRunning = True
                        elif newBar.low < stopLossForLong:
                            result-= 1
                            return result
                    elif shortEntryRunning:
                        if newBar.low < profitTargetForShort:
                            result += 1
                            shortEntryRunning = False
                            longEntryRunning = True
                        elif newBar.high > stopLossForShort:
                            result-= 1
                            return result
